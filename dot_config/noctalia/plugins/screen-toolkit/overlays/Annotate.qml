...
Variants {
    ...
    delegate: PanelWindow {
                ...
                if (code === 0) {
                    ...
                    if (dest !== "")
                        wlCopyUrlProc.exec({ command: ["bash", "-c",
                            "printf '%s' " + U.shellEscape(dest) + " | wl-copy"] })
        ...
        Shortcut {
            ...
            onActivated: {
                if (overlayWin.isSaving || !overlayWin.isPrimary) return
                overlayWin.flattenAndSave()
            }
        }
        // --- Tool selection shortcuts ---
        // Disabled while typing text (so letters go to the text field) and while zoomed.
        property bool _toolKeysEnabled: root.isVisible && overlayWin.isPrimary
            && !overlayWin.textMode && !overlayWin.isSaving && root.zoomScale <= 1.0
        Shortcut { sequence: "P"; enabled: overlayWin._toolKeysEnabled; onActivated: overlayWin.tool = "pencil" }
        Shortcut { sequence: "H"; enabled: overlayWin._toolKeysEnabled; onActivated: overlayWin.tool = "highlighter" }
        Shortcut { sequence: "L"; enabled: overlayWin._toolKeysEnabled; onActivated: overlayWin.tool = "line" }
        Shortcut { sequence: "A"; enabled: overlayWin._toolKeysEnabled; onActivated: overlayWin.tool = "arrow" }
        Shortcut { sequence: "R"; enabled: overlayWin._toolKeysEnabled; onActivated: overlayWin.tool = "rect" }
        Shortcut { sequence: "C"; enabled: overlayWin._toolKeysEnabled; onActivated: overlayWin.tool = "circle" }
        Shortcut { sequence: "T"; enabled: overlayWin._toolKeysEnabled; onActivated: overlayWin.tool = "text" }
        ...
        Shortcut {
            sequence: "Ctrl+Z"
            enabled: root.isVisible && overlayWin.isPrimary && !overlayWin.isSaving && root.zoomScale <= 1.0
            onActivated: toolbar.doUndo()
        }
        Shortcut {
            sequences: ["Ctrl+Shift+Z", "Ctrl+Y"]
            enabled: root.isVisible && overlayWin.isPrimary && !overlayWin.isSaving && root.zoomScale <= 1.0
            onActivated: toolbar.doRedo()
        }
        Process {
