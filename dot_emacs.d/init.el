...
(use-package emacs
  ...
  (truncate-lines nil)                            ;; Disable truncation so lines wrap.
  (word-wrap t)                                   ;; Wrap at word boundaries, not mid-word.
...
;;; POPPER
;; Manage popup windows without disrupting your window layout
(use-package popper
  :ensure t
  :straight t
  :init
  ...
  :bind
  (("C-`"   . popper-toggle)           ;; Toggle last popup
...
(use-package org
  ...
  (setq org-todo-keywords
        '((sequence "TODO(t)" "WIP(p)" "REVIEW(r)" "BLOCKED(b)" "|" "DONE(d)" "CANCELED(c)" "HOLD(h)")
          (sequence "QUESTION(q)" "|" "ANSWER(a)")))
          ;; (sequence "BUG(b)" "BUGWIP(w)" "KNOWNCAUSE(k)" "|" "FIXED(f)")))
...
;;; BLAMER
;; Show git blame info as virtual text after current line
(use-package blamer
  :ensure t
  :straight t
  :defer t
  :custom
  (blamer-idle-time 1.0)                 ;; Show after 1 second
  (blamer-min-offset 50)                 ;; Minimum offset from line content
  (blamer-author-formatter " ✎ %s ")    ;; Author format
  (blamer-datetime-formatter "[%s]")    ;; Date format
  (blamer-commit-formatter " ● %s")     ;; Commit message format
  (blamer-prettify-time-p t)            ;; Show "2 days ago" instead of date
  :custom-face
...
(use-package evil
  ...
  (defun ek/show-diagnostic-at-point ()
    "Show flymake diagnostic at current cursor position."
    (interactive)
    (let ((diags (flymake-diagnostics (point))))
      (if diags
          (message "%s" (flymake-diagnostic-text (car diags)))
        ;; Try to find diagnostic on current line if not exactly at point
        (let ((line-diags (flymake-diagnostics (line-beginning-position) (line-end-position))))
          (if line-diags
              (message "%s" (flymake-diagnostic-text (car line-diags)))
            (message "No diagnostic at point"))))))
  (evil-define-key 'normal 'global (kbd "<leader> k") 'ek/show-diagnostic-at-point)
  ...
  ;; Move by visual lines (wrapped lines) instead of actual lines
  ...
  ;; Select last pasted text (like gp in Neovim)
  (evil-define-key 'normal 'global (kbd "gp")
                   (lambda ()
                     (interactive)
                     (evil-goto-mark ?\[)
                     (evil-visual-char)
...
(use-package org-modern
  ...
  (setq org-modern-todo-faces
        '(("TODO"     :background "#ff6b6b" :foreground "#ffffff" :weight bold)
          ("WIP"      :background "#4ecdc4" :foreground "#1a1a2e" :weight bold)
          ("REVIEW"   :background "#a29bfe" :foreground "#1a1a2e" :weight bold)
          ("BLOCKED"  :background "#fd79a8" :foreground "#1a1a2e" :weight bold)
          ("HOLD"     :background "#95a5a6" :foreground "#ffffff" :weight bold)
          ("DONE"     :background "#2ecc71" :foreground "#ffffff" :weight bold)
          ("CANCELED" :background "#636e72" :foreground "#dfe6e9" :weight bold)
          ("QUESTION" :background "#fdcb6e" :foreground "#1a1a2e" :weight bold)
          ("ANSWER"   :background "#00b894" :foreground "#ffffff" :weight bold)))
...
;; Run sb CLI after saving inbox.org to process captures, then commit results.
;; This runs AFTER git-auto-commit-mode commits the raw capture.
;; Two separate commits: one for raw capture, one for processed state.
;;
;; Sequence:
;; 1. User saves inbox.org
;; 2. git-auto-commit commits raw capture
;; 3. sb processes inbox (async, fire-and-forget)
;; 4. On sb completion, commit + push processed changes

;; Ensure sb binary is in exec-path (installed via: go install ./cmd/sb/)
(add-to-list 'exec-path (expand-file-name "~/go/bin"))

(defun ek/second-brain-process-and-commit ()
  "Run sb CLI after inbox.org save, then commit and push processed changes."
  (when (and buffer-file-name
             (string-equal (file-truename buffer-file-name)
                           (file-truename (expand-file-name "~/org/SecondBrain/inbox.org"))))
    (let ((default-directory (expand-file-name "~/org/SecondBrain/")))
      (message "Second Brain: Processing inbox...")
      (let ((proc (start-process "sb-process" "*sb-output*"
                                 (expand-file-name "~/go/bin/sb")
                                 "-inbox" (expand-file-name "~/org/SecondBrain/inbox.org")
                                 "-v")))
        (set-process-sentinel proc #'ek/second-brain-process-sentinel)))))

(defun ek/second-brain-process-sentinel (proc event)
  "Handle sb process completion: commit and push if successful."
  (let ((default-directory (expand-file-name "~/org/SecondBrain/"))
    ...
    (cond
     ...
     ((and (string-match-p "finished" event) (= exit-code 0))
      (message "Second Brain: Processing complete (exit 0)")
      ;; Log output for debugging
      (when (> (length output) 0)
        (message "Second Brain output: %s" (substring output 0 (min 200 (length output)))))
      ;; Only commit if there are changes
      (ek/second-brain-commit-if-changes))
     ;; Non-zero exit or error
     (t
      (message "Second Brain: Failed (exit %d) - %s" exit-code (string-trim event))
      (when (> (length output) 0)
        (message "Second Brain error output: %s" output))))))

(defun ek/second-brain-commit-if-changes ()
  "Commit and push only if there are uncommitted changes."
  (let ((default-directory (expand-file-name "~/org/SecondBrain/")))
    (set-process-sentinel
     (start-process "sb-git-status" "*sb-git*" "git" "status" "--porcelain")
     (lambda (proc _event)
       (let ((status-output (with-current-buffer (process-buffer proc)
                              (string-trim (buffer-string)))))
         (if (> (length status-output) 0)
             ;; There are changes, commit them
             (progn
               (message "Second Brain: Changes detected, committing...")
               (set-process-sentinel
                (start-process "sb-git-add" nil "git" "add" "-A")
                (lambda (_proc _event)
                  (set-process-sentinel
                   (start-process "sb-git-commit" nil
                                  "git" "commit" "-m" "sb: processed inbox")
                   (lambda (_proc _event)
                     (start-process "sb-git-push" nil "git" "push")
                     (message "Second Brain: Committed and pushed."))))))
           ;; No changes
...
;; slick - edit your filesystem like a buffer (oil.nvim for Emacs)
;; Built to address gaps in grease.el:
;; - Opens with `-` overlay on current buffer (like oil.nvim)
;; - Native yy/p for cross-buffer file moves
;; - Hidden files visible by default
(add-to-list 'load-path "~/projects/emacs_plugins.git/main/slick")
(require 'slick)

;; slick keybindings (Evil):
;; - `-` (global): Open slick for current file's directory
;; - `-` (in slick): Go to parent directory
;; - RET: Open file/directory under cursor
;; - q: Quit and restore previous buffer
;; - gr: Refresh
;; - g.: Toggle hidden files
;; - :w / C-c C-c: Save (apply changes with confirmation)
